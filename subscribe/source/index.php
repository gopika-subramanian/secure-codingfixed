...
<!-- / menu --> 
  ...
  <!-- / catg header banner section -->
    ...
    <div class="splash">
        <h1 class="splash-head">ENTER IP to subscribe</span></h1>

        <form method="POST" class="pure-form">
            <input name="command" type="text" class="pure-input-1" id="ip" />
           
            <button type="submit" name="ping" class="pure-button pure-button-primary button-large">Search!</button>
        </form>
        <h3>Results</h3> 
...
if (isset($_POST["command"])) {
  ...
  $command = validateAndSanitize($command);

  if ($command !== false) {
      $output = runSafeCommand($command);
...
function validateAndSanitize($input) {
  ...
  // Validate the input against the allowed pattern.
  if (preg_match($allowedPattern, $input) === 1) {
      // Input is safe; return the sanitized input.
